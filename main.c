...
int add_route(int argc, char **argv) {
	...
	if (NgSendMsg(csock, path, NGM_GENERIC_COOKIE, NGM_MKPEER, &mkp,
			sizeof(mkp)) < 0) {
		fprintf(stderr, "main(): mkpeer error  :%s\n", strerror(errno));
		return (0);
	}
	...
	if (NgSendMsg(csock, path, NGM_KSOCKET_COOKIE, NGM_KSOCKET_CONNECT,
		...
		fprintf(stderr, "%s: CONNECT to %s:%d FAILED %s\n", __FUNCTION__,
				inet_ntoa(cfg.dst.sin_addr), cfg.dst.sin_port, strerror(errno));
		return 0;
	}
...
void show_routes(void) {
	...
	while (nlist->numnames > 0) {
		if (regexec(preg, ninfo->name, 0, NULL, 0) == 0) {
			...
			if (NgSendMsg(csock, path, NGM_KSOCKET_COOKIE, NGM_KSOCKET_GETNAME,
					NULL, 0) < 0) {
				printf("%s(): line: %d %s\n", __FUNCTION__, __LINE__,
						strerror(errno));
				exit(EXIT_FAILURE);
